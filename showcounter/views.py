...
def overview(request):
    ...
    productions = Production.objects.annotate(
        num_participations=Count(
            "performances", filter=Q(performances__participants__person=user.person)
        )
    ).order_by("-number")
    total_participations = Participation.objects.filter(person=user.person).count()

    context = {"productions": productions, "total_participations": total_participations}
...
def production(request, number):
    ...
    with transaction.atomic():
        ...
        PerformanceFormSet = formset_factory(extra=0, form=PerformanceForm)
        context = {
            ...
            for form in performance_formset:
                ...
                if form.cleaned_data["participated"]:
                    ...
                    Participation.objects.get_or_create(
                        performance=performance, person=user.person
                    )
                else:
                    Participation.objects.filter(
                        performance__number=number, person=user.person
                    ).delete()
            return redirect("showcounter:overview")

        performances = Performance.objects.filter(production=production).order_by(
            "date"
        )
        initial = []
        for performance in performances:
            current_user_participated = performance.participants.filter(
                person__member_number=user.person.member_number
            ).exists()
            initial.append(
                {
