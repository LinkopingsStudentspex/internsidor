...
def activation_view(request):
    ...
    if bad_token:
        return HttpResponse(
            "Du har nog följt en gammal länk eller så har den kopierats fel"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if (
                ...
                and activation.provision_gsuite_account
            ):
                gsuite.create_user(
                    person.user.username, person.first_name, person.last_name
                )
...
class PersonSelfView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "object" in context:
            context["extra_emails"] = models.ExtraEmail.objects.filter(
...
class StatisticsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        honorary = models.AssociationMembership.objects.filter(
            membership_type="HON", person__deceased=False
        ).values_list("person", flat=True)

        lifetime = (
            models.AssociationMembership.objects.filter(
                membership_type="LIF", person__deceased=False
            )
        ...
        regular_productions = (
            models.Production.objects.filter(regular=True)
            .exclude(number__in=not_really_productions)
            .count()
        )
        extra_productions = (
            models.Production.objects.filter(regular=False)
            .exclude(number__in=not_really_productions)
            .count()
        )
        context["productions"] = {
...
def email_list_filter(request):
    context = {}
    context["filter"] = EmailListFilter(
        request.GET, queryset=models.EmailList.objects.all()
    )
    context["email_domain"] = settings.EMAIL_DOMAIN
...
admin.site.login = login_required(
    staff_member_required(admin.site.login, login_url="batadasen:no_admin")
)
