...
from django.contrib.auth.admin import (
    UserAdmin as origUserAdmin,
    GroupAdmin as origGroupAdmin,
)
...
class ProductionMembershipInline(admin.TabularInline):
    model = models.ProductionMembership
    extra = 0
...
class AssociationMembershipInline(admin.TabularInline):
    model = models.AssociationMembership
    extra = 0


class AssociationActivityInline(admin.TabularInline):
    model = models.AssociationActivity
    extra = 0
...
class PersonForm(forms.ModelForm):
    ...
    send_activation_link = forms.BooleanField(
        required=False, label="Skicka aktiveringslänk för att skapa ny användare."
    )
    provision_gsuite_account = forms.BooleanField(
        required=False, label="Aktivera Google-konto för användaren."
    )
    ...
    def clean(self):
        ...
        if self.instance.user is not None and cleaned_data["send_activation_link"]:
            self.add_error(
                ...
                "Personen har redan en aktiv användare. Be en administratör ta bort användaren först!",
            )

        requested_gsuite_activation = (
            "provision_gsuite_account" in cleaned_data
            and cleaned_data["provision_gsuite_account"]
        )

        if (
            requested_gsuite_activation
            and self.instance.user is None
            and not cleaned_data["send_activation_link"]
        ):
            self.add_error(
                "provision_gsuite_account",
                "Personen har ingen användare och du har inte klickat i att en ny användare ska skapas.",
            )

        if requested_gsuite_activation and not settings.PROVISION_GSUITE_ACCOUNTS:
            self.add_error(
                "provision_gsuite_account",
                "Aktivering av Google-konton är inaktiverat.",
            )
...
class PersonAdmin(admin.ModelAdmin):
    ...
    def get_production_groups(self, obj):
        group_list = []
        count = 0
        for membership in obj.production_memberships.order_by(
            "-group__production__year"
        ):
            count += 1
            if count < 6:
    ...
    def save_model(self, request, obj, form, change):
        ...
        if form.cleaned_data["send_activation_link"]:
            ...
            existing_user_activations = models.UserActivation.objects.filter(person=obj)
            ...
            activation = models.UserActivation(
                person=obj,
                provision_gsuite_account=form.cleaned_data["provision_gsuite_account"],
            )
            activation.save()

            mail.send_mail(
                "Aktiveringslänk för spexets internsidor",
                render_to_string(
                    template_name="batadasen/activation_email.txt",
                    context={
                        "first_name": activation.person.first_name,
                        "activation_url": request.build_absolute_uri(
                            reverse("batadasen:activate") + "?token=" + activation.token
                        ),
            ...
            messages.info(
                request,
                "Ett mail med en aktiveringslänk har skickats till {}. Länken går ut {}".format(
                    email, activation.valid_until.strftime("%Y-%m-%d %H:%M:%S")
                ),
            )
        elif (
            settings.PROVISION_GSUITE_ACCOUNTS
            and form.cleaned_data["provision_gsuite_account"]
...
class EmailListAdmin(admin.ModelAdmin):
    fieldsets = (
        ...
        ("Övriga val", {"fields": ("all_titles",), "classes": ("collapse",)}),
    )
    filter_horizontal = (
        "opt_in_members",
        "opt_out_members",
...
class ProductionGroupAdmin(admin.ModelAdmin):
    ...
    search_fields = [
        "group_type__short_name",
        "group_type__name",
        "production__short_name",
    ]
...
admin.site.register(models.AssociationActivity)
admin.site.register(models.AssociationMembership, AssociationMembershipAdmin)
admin.site.register(models.AssociationYear)
admin.site.register(models.EmailList, EmailListAdmin)
admin.site.register(models.ExtraEmail)
admin.site.register(models.AssociationGroupType, GenericGroupAdmin)
admin.site.register(models.AssociationGroup)
admin.site.register(models.ProductionGroupType, ProductionGroupTypeAdmin)
admin.site.register(models.Person, PersonAdmin)
admin.site.register(models.Production)
admin.site.register(models.ProductionGroup, ProductionGroupAdmin)
