...
from .models import LogEntry, Asset, Owner, get_next_asset_number
...
class AssetModelForm(forms.Form):
    ...
    model_description = forms.CharField(
        label=_("Model description"), widget=forms.Textarea(attrs={"rows": 2})
    )
...
class LogEntryForm(forms.Form):
    ...
    new_status = forms.ChoiceField(
        label=_("New status"), choices=LogEntry.STATUS_CHOICES
    )
...
class AssetForm(forms.Form):
    number = forms.IntegerField(
        label=_("Number"), min_value=1, initial=get_next_asset_number
    )
    ...
    purchase_time = forms.CharField(
        label=_("Purchase time"), max_length=100, required=False
    )
    purchase_time.widget.attrs["class"] = "form-control"

    purchase_price = forms.CharField(
        label=_("Purchase price"), max_length=100, required=False
    )
    purchase_price.widget.attrs["class"] = "form-control"

    standard_location = forms.CharField(
        label=_("Standard location"), max_length=100, required=False
    )
    ...
    initial_status = forms.ChoiceField(
        ...
        choices=LogEntry.STATUS_CHOICES,
        initial=LogEntry.STATUS_UNKNOWN,
    )
    initial_status.widget.attrs["class"] = "form-control"

    initial_log_entry = forms.CharField(
        label=_("Initial log entry"), widget=forms.Textarea(attrs={"rows": 2})
    )
    ...
    def clean_number(self):
        ...
        if Asset.objects.filter(number=data).exists():
            raise forms.ValidationError(
                _("Another asset already has this number: ") + str(data)
            )
