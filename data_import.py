...
with open(input_filename) as json_file:
    with transaction.atomic():
        ...
        for json_extramail in data['extramail']:
            ...
            person = models.Person.objects.get(member_number=json_extramail['medlem'])
            email, created = models.ExtraEmail.objects.get_or_create(person=person, email=json_extramail['email'])
            current += 1
        ...
        for json_grupp in data['grupp']:
            ...
            if json_grupp['kortnamn'] in ASSOC_GROUPS:
                group_type, created = models.AssociationGroupType.objects.get_or_create(short_name=json_grupp['kortnamn'])
                if created:
            ...
            if json_grupp['kortnamn'] in PROD_GROUPS:
                group_type, created = models.ProductionGroupType.objects.get_or_create(short_name=json_grupp['kortnamn'])
                if created:
                    group_type.name = json_grupp['namn']
                    group_type.priority = json_grupp['prio']
                    group_type.save()
        ...
        for json_titel in data['titel']:
            ...
            title, created = models.Title.objects.get_or_create(name=json_titel['namn'])
            if created:
        ...
        for json_foreningsaktiv in data['foreningsaktiv']:
            ...
            assoc_group_type = models.AssociationGroupType.objects.get(short_name=json_foreningsaktiv['grupp'])
            assoc_year, created = models.AssociationYear.objects.get_or_create(end_year=json_foreningsaktiv['ar'])
            assoc_group, created = models.AssociationGroup.objects.get_or_create(year=assoc_year, group_type=assoc_group_type)
            person = models.Person.objects.get(member_number=json_foreningsaktiv['medlem'])
            activity, created = models.AssociationActivity.objects.get_or_create(person=person, group=assoc_group)
            if created:
                if json_foreningsaktiv['titel'] is not None:
                    title = models.Title.objects.get(name=json_foreningsaktiv['titel'])
        ...
        for json_uppsattning in data['uppsattning']:
            ...
            production, created = models.Production.objects.get_or_create(number=json_uppsattning['nr'], main_title=json_uppsattning['namn'], year=json_uppsattning['ar'])
            if created:
        ...
        for json_gruppstatus in data['gruppstatus']:
            ...
            person = models.Person.objects.get(member_number=json_gruppstatus['medlem'])
            try:
                # Need to use an inner transaction here since the exception will break the closest
                # transaction if it happens.
                with transaction.atomic():
                    title = models.Title.objects.get(name=json_gruppstatus['titel'])
            except models.Title.DoesNotExist:
                title = None
            
            production = models.Production.objects.get(number=json_gruppstatus['uppsattning'])
            group_type = models.ProductionGroupType.objects.get(short_name=json_gruppstatus['grupp'])
            prod_group, created_group = models.ProductionGroup.objects.get_or_create(production=production, group_type=group_type)
            production_membership, created_membership = models.ProductionMembership.objects.get_or_create(person=person, group=prod_group, title=title)
        ...
        for json_mejllistor in data['mejllistor']:
            ...
            maillist, created = models.EmailList.objects.get_or_create(alias=json_mejllistor['alias'])
            if json_mejllistor['uppsattning'] is not None:
                production = models.Production.objects.get(number=json_mejllistor['uppsattning'])
            else:
                production = None
            
            if json_mejllistor['grupp'] is not None and json_mejllistor['grupp'].strip() != '':
                group_type, created = models.ProductionGroupType.objects.get_or_create(short_name=json_mejllistor['grupp'])
                if created:
            ...
            if json_mejllistor['extramedlem'] is not None:
                extra_person = models.Person.objects.get(member_number=json_mejllistor['extramedlem'])
            else:
                extra_person = None
     
            if json_mejllistor['optout'] is not None:
                optout_person = models.Person.objects.get(member_number=json_mejllistor['optout'])
            else:
                optout_person = None
        ...
        for json_password in data['password']:
            ...
            if json_password['username'] is not None:
                ...
                if created:
                    person = models.Person.objects.get(member_number=json_password['medlem'])
                    person.user = user
                    person.save()
            current += 1
